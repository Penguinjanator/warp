...
class ChatFormat:
    ...
    def load(cls, engine: Any) -> "ChatFormat":
        ...
        extra = [prelude, effort, image, stop_marker] + list(think or ())
        ids: dict[str, int] = {}
    ...
    def build_chat_segments(self, messages: list[Any],
        ...
        for i, message in enumerate(messages):
            ...
            if role == "tool":
                if not self.tool_markers:
                    raise ChatFormatError(
                        f"messages[{i}] is a tool result, but this container "
                        "has no supported tool protocol", param=f"messages[{i}].role")
                if self.tool_protocol == "glm":
            ...
            if tool_calls:
                if not self.tool_markers:
                    raise ChatFormatError(
                        f"messages[{i}] carries tool_calls, but this container "
                        "has no supported tool protocol",
