...
# The system text that follows the `<|system|>` opener. The header is
# ordinary prose; the footer's example includes reserved XML tokens and
# is encoded as template markup. Caller signatures remain plain text.
_DECL_HEADER = (
    "\n# Tools\n\n"
...
def _function(tool: dict, index: int) -> dict:
    ...
    if not isinstance(fn, dict) or not isinstance(fn.get("name"), str) or not fn["name"]:
        raise GlmToolError("each tool must name a function", param=f"tools[{index}]")
    parameters = fn.get("parameters", {})
    if (not isinstance(parameters, dict)
            or not isinstance(parameters.get("properties", {}), dict)):
        raise GlmToolError("tool parameters and properties must be objects",
...
def declaration(tools: list[dict]) -> list[Segment]:
    ...
    for index, tool in enumerate(tools):
        fn = _function(tool, index)
        if fn.get("defer_loading"):
            raise GlmToolError("deferred tool loading is not supported", param="tools")
        fn = {k: v for k, v in fn.items() if k not in ("defer_loading", "strict")}
        out.append(Segment("\n" + json.dumps(fn, ensure_ascii=False) + "\n\n"))
    # This example belongs to the template: its reserved XML tags must be
    # encoded as markup just like the tags in an actual tool call.
    out.append(Segment(_DECL_FOOTER, markup=True))
    return out
...
class ToolParser:
    ...
    def __init__(self, tools: Optional[list[dict]] = None) -> None:
        ...
        functions = [_function(t, i) for i, t in enumerate(tools or [])]
        self._schemas = {fn["name"]: fn.get("parameters", {}).get("properties", {})
                         for fn in functions}
        self._state = "content"
        self._key = ""
        self._value = ""
    ...
    def _finish_arg(self) -> None:
        if self._current is not None and self._key:
            key = self._key.strip()
            value = self._value
            schema = self._schemas.get(self._current.name.strip(), {}).get(key, {})
            kind = schema.get("type") if isinstance(schema, dict) else None
            # String arguments such as "001" must stay strings; JSON values
            # are decoded only when the declared schema asks for them.
            if kind in ("integer", "number", "boolean", "object", "array", "null"):
                try:
                    value = json.loads(value)
                except ValueError:
                    pass
            self._current.arguments[key] = value
        self._key = ""
        self._value = ""
