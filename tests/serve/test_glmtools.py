...
class TestLoad(Base):
    ...
    def test_unsupported_tool_history_is_refused_without_definitions(self):
        fmt = self.load(markers={k: v for k, v in GLM_MARKERS.items()
                                 if v not in glmtools.MARKERS})
        for message in [CALL, {"role": "tool", "content": "18C"}]:
            with self.subTest(role=message["role"]):
                with self.assertRaises(ChatFormatError):
                    fmt.build_chat_segments([message], thinking=True)
...
class TestGlmReplyParser(unittest.TestCase):
    ...
    def test_schema_types_and_name_whitespace(self):
        tools = [{"type": "function", "function": {
            "name": "lookup", "parameters": {"properties": {
                "count": {"type": "integer"}, "code": {"type": "string"},
                "enabled": {"type": "boolean"}, "items": {"type": "array"}}}}}]
        p = PlainParser(markers=self.MARKERS,
                        tool_parser=glmtools.ToolParser(tools))
        self.feed(p, [(21, "<tool_call>"), (2000, "lookup\n")])
        for key, value in [("count", "3"), ("code", "001"),
                           ("enabled", "true"), ("items", '[1, "x"]')]:
            self.feed(p, [(23, ""), (2000, key), (24, ""), (2000, "\n"),
                          (25, ""), (2000, value), (26, "")])
        self.feed(p, [(22, "")])
        self.assertEqual(p.tool_calls[0].name, "lookup")
        self.assertEqual(p.tool_calls[0].arguments,
                         {"count": 3, "code": "001", "enabled": True,
                          "items": [1, "x"]})
